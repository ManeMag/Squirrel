...
class StatisticIncome(
    private val transactions: List<Transaction>,
    private val impact: List<Impact>
) :
    Fragment(R.layout.fragment_statistic_income) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            val response: HttpResponse =
                client.get("$protocol://$domain:$port/api/category/1")
            ...
            listCategory.indices.forEach {
                ...
                with(RadioButton(layout.context)) {
                    ...
                    val gradientDrawable = getDrawable(
                        requireContext(),
                    ...
                    setPadding(15)
                    background = getDrawable(context, R.drawable.radio_button_category)
    ...
    private fun setData(mChart: PieChart) {
        ...
        with(values) {
            impact.forEach {
                if(listCategory.any{category -> category.id == it.categoryId } && it.positivePercentage > 0){
                    add(PieEntry(it.positivePercentage.toFloat()))
